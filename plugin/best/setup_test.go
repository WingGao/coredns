...
func TestSetupChaos(t *testing.T) {
	...
	for i, test := range tests {
		...
		if !test.shouldErr && version != test.expectedVersion {
			t.Errorf("Test %d: Best not correctly set for input %s. Expected: %s, actual: %s", i, test.input, test.expectedVersion, version)
		}
		if !test.shouldErr && authors != nil && test.expectedAuthor != "" {
			if authors[0] != test.expectedAuthor {
				t.Errorf("Test %d: Best not correctly set for input %s. Expected: '%s', actual: '%s'", i, test.input, test.expectedAuthor, authors[0])
			}
		}
	}
}
